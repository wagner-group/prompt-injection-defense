...
"""
    ...
    if example is not None:
        template += f"For example, input number 1 is:\n### 1. {example.strip()}\n\n\nPlease follow the same format, but make sure to generate different content.\n"
...
def get_oneshot_prompt(
    index, task, example, additional_rules=None, random_seed=None
):
    """
    Returns a prompt for the user to generate inputs for a language model fine-tuning dataset.
...
Let's build this dataset together."""
    ...
    template = f"""
This language model will be fine-tuned for a specific prompt by providing it with input-outputs pairs.
The prompt is "{task.strip()}". 
I will need for you to think of specific unique, diverse, long-form and realistic inputs.
I will write the outputs for these inputs myself, you just need to think of inputs.
Since your context length is not long enough, I will be querying inputs one by one.
...
Here is an example of an input for this task. Your inputs can vary the style and format a bit. Your inputs must have a similar or higher level of detail, complexity, and realism, and must use diverse topics:
